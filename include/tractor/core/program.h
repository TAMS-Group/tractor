...
class Program {
 public:
  ...
  template <class Impl>
  class BindablePortBase : public PortBase<Impl> {
   ...
   public:
    inline Input(TypeInfo type, uintptr_t address, uintptr_t offset,
        ...
        : BindablePortBase<Input>(type, address, offset, binding),
          ...
          _upper_bound(upper_bound),  //_phase(phase)
          _mode(mode) {}
 ...
 public:
  ...
  template <class List>
  inline void setInputs(const List &inputs) {
    _inputs.clear();
  ...
  template <class List>
  inline void setConstants(const List &constants) {
    _constants.clear();
