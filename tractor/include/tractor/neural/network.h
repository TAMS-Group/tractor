...
template <class Scalar> class NeuralNetwork : public NeuralBase {
  ...
  static void
  _findLayerSet(const std::shared_ptr<Layer<Scalar>> &layer,
  ...
  static void
  _evaluate(const std::shared_ptr<Layer<Scalar>> &layer, const LayerMode &mode,
            std::unordered_map<std::shared_ptr<Layer<Scalar>>, Tensor<Scalar>>
                &tensors) {
