...
public:
  static const std::shared_ptr<MessageType> &
  instance(const std::string &name, const std::string &hash,
           const std::string &definition);
...
public:
  /*
  template <class T> Message(const T &m) {
    static auto type =
        MessageType::instance(ros::message_traits::DataType<T>::value(m),
                              ros::message_traits::MD5Sum<T>::value(m),
                              ros::message_traits::Definition<T>::value(m));
    _type = type;
    ros::serialization::OStream strm;
    ros::serialization::serialize(strm, m);
    _data.assign(strm.getData(), strm.getData() + strm.getLength());
  }
  */
...
template <> struct MD5Sum<tractor::Message> {
  static const char *value() { return "*"; }
  static const char *value(const tractor::Message &message) {
    return message.type()->hash().c_str();
  }
};
template <> struct DataType<tractor::Message> {
  static const char *value() { return "*"; }
  static const char *value(const tractor::Message &message) {
    return message.type()->name().c_str();
  }
};
template <> struct Definition<tractor::Message> {
  static const char *value() { return "*"; }
  static const char *value(const tractor::Message &message) {
...
template <> struct Serializer<tractor::Message> {
