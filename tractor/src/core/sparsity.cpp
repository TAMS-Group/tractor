...
template <class PortArray> static size_t findPortSize(const PortArray &ports) {
  size_t size = 0;
  for (auto &port : ports) {
...
template <class Mask> class SparsityVector {
...
template <class Mask>
static SparsityVector<Mask>
propagateSparsity(const Program &program,
                  const SparsityVector<Mask> &input_sparsity) {
...
template <class Mask> inline Mask makeMask(size_t i) {
