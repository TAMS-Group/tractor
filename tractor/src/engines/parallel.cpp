...
void ParallelEngine::ExecutableImpl::_compile(const Program &program) {
  ...
  for (size_t iw = 0; iw < _waves.size(); iw++) {
    auto &wave = _waves[iw];
    std::cout << "wave " << iw << " " << wave.instructions.size() << std::endl;
  }
}
...
#pragma omp parallel for
        for (size_t i = 0; i < n; i++) {
          ...
          inst.op(temp.data(), arguments + inst.base);
        }
      }
    }
...
#pragma omp parallel
      {
        ...
        for (auto &wave : _waves) {
          auto *instructions = wave.instructions.data();
          auto *arguments = wave.arguments.data();
          size_t istart = wave.instructions.size() * (ti + 0) / tn;
          size_t istop = wave.instructions.size() * (ti + 1) / tn;
