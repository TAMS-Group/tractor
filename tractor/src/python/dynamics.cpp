...
static void pythonizeDynamics(py::module main_module, py::module type_module) {
  ...
  py::class_<RigidBody<Geometry>>(type_module, "RigidBody")
      ...
      //         &RigidBody<Geometry>::pose))
      //
      // .def_property(
      //     "position",
      //     [](const RigidBody<Geometry> &_this) { return _this.position(); },
      //     py::overload_cast<const typename Geometry::Vector3 &>(
      //         &RigidBody<Geometry>::position))
      //
      // .def_property(
      //     "orientation",
      //     [](const RigidBody<Geometry> &_this) { return _this.orientation();
      //     }, py::overload_cast<const typename Geometry::Orientation &>(
      //         &RigidBody<Geometry>::orientation))
