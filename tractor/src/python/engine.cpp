...
static void pythonizeEngine(py::module &m) {
  ...
  py::class_<Executable, std::shared_ptr<Executable>>(m, "Executable")
      ...
      //         const std::shared_ptr<Memory> &memory) {
      //        executable->inputVector(data, memory);
      //      })
      .def("output",
           [](const std::shared_ptr<Executable> &executable,
              const std::shared_ptr<Memory> &memory /*, py::array buffer*/) {
             /*if (buffer.nbytes() < executable->outputBufferSize()) {
               throw std::runtime_error("output buffer too small");
             }
             Buffer b;
             executable->output(memory, b);
             std::memcpy(buffer.mutable_data(), b.data(),
                         executable->outputBufferSize());*/
             Buffer b;
             executable->output(memory, b);
