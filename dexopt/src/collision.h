...
//
//     auto &pose_a =
//     value(trajectory.state(_frame_index).links().pose(_link_a)); auto &pose_b
//     = value(trajectory.state(_frame_index).links().pose(_link_b));
//
...
//     CollisionShape(shapes::ShapeConstPtr shape, const Eigen::Affine3d
//     &origin) {
...
//             auto &pose_b =
//             trajectory.state(iframe).links().pose(pair.second);
//
